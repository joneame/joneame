...
function get_votes($dbtime, $link_id) {
    ...
    $res = $db->get_results("select vote_id, unix_timestamp(vote_date) as timestamp, vote_value, INET_NTOA(vote_ip_int) as vote_ip, vote_user_id, link_id, link_date, link_votes, link_anonymous, link_status, link_comments from votes, links where vote_type='links' and vote_link_id = $link_id and UNIX_TIMESTAMP(vote_date) > ".$last_timestamp." and link_id = vote_link_id order by vote_date desc limit $max_items");
    if (!$res) return;
...
function get_comment($dbtime, $linkid) {
    ...
    $res = $db->get_results("select unix_timestamp(comment_date) as timestamp, user_id, user_login, comment_user_id, comment_type, comment_order, link_id, link_date, link_votes, link_anonymous, link_status, link_comments, comment_id from comments, links, users where link_id = $linkid and comment_link_id =$linkid and user_id = comment_user_id and UNIX_TIMESTAMP(comment_date) > ".$last_timestamp." order by comment_date desc limit $max_items");
    if (!$res) return;
    foreach ($res as $event) {
...
function get_status($status) {
    switch ($status) {
        case 'published': //editar aqui para editar el estado
            $status = _('en portada');
            break;
        case 'queued':
            $status = _('pendiente');
