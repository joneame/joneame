...
if ($dbusers) {
    ...
    foreach($dbusers as $dbuser) {
        ...
        echo '<strong>'.$friend->username.'</strong></a>';
        if ($current_user->user_id > 0 && $current_user->user_id != $friend->id) {
            echo '<a class="selector-amigo" id="friend-'.$prefered_type.'-'.$current_user->user_id.'-'.$friend->id.'" href="javascript:obtener(\'amigos.php\',\''.$current_user->user_id.'\',\'friend-'.$prefered_type.'-'.$current_user->user_id.'-'.$friend->id.'\',0,\''.$friend->id.'\')">'.friend_teaser($current_user->user_id, $friend->id).'</a>';
        }
        echo '</div>';
    }
    echo "<br clear='left'/>\n";
    do_contained_pages($prefered_id, $prefered_total, $prefered_page, $prefered_page_size, 'paginas_amigos.php', $prefered_type, $prefered_type.'-container');
}