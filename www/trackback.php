...
if(check_ban($globals['user_ip'], 'ip')) {
	syslog(LOG_NOTICE, "Joneame: trackback, IP is banned: $urlfrom[host], $globals[user_ip]");
	trackback_response(1, 'Server banned.');
}

if(check_ban($urlfrom[host], 'hostname', false)) {
	syslog(LOG_NOTICE, "Joneame: trackback, server is banned: $urlfrom[host]");
	trackback_response(1, 'Server banned.');
}
...
if ($link->date < (time() - 86400*7)) {
	syslog(LOG_NOTICE, "Joneame: Trackback en historia demasiado antigua: $tb_url -> " . $link->get_permalink());
	die;
}
...
if ( !empty($tb_url) && !empty($title) && !empty($excerpt) ) {
	...
	$dupe = $trackres->read();
	if ( $dupe ) {
		syslog(LOG_NOTICE, "Joneame: We already have a ping from that URI for this post: $tb_url - $permalink");
		trackback_response(1, 'We already have a ping from that URI for this post.');
	}
  
	$contents=@file_get_contents($tb_url);
	if(!$contents) {
		syslog(LOG_NOTICE, "Joneame: The provided URL does not seem to work: $tb_url");
	...
	if(!preg_match($pattern,$contents)) {
		syslog(LOG_NOTICE, "Joneame: The provided URL does not have a link back to us: $tb_url");
	...
	$trackres->status='ok';
	$trackres->store();
	syslog(LOG_NOTICE, "Joneame: trackback ok: $tb_url - $permalink");
